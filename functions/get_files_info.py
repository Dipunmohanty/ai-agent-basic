...
def get_files_info(working_directory: str, directory: str = ".") -> list[str]|str:
    ...
    try:    
        ...
        for dir in os.listdir(target_dir):
            item = os.path.join(target_dir,dir)
            folder.append(f"{item}: file_size={os.path.getsize(item)} bytes, is_dir={os.path.isdir(item)}")
        info = "- " + "\n - ".join(folder)
        return info
